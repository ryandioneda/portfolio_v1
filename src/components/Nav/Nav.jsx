...
const Nav = () => {
    ...
    return (
        <div id='nav-wrapper' className="bg-offblack position: fixed w-full p-2.5 font-inter sm:bg-gray-400">
            <nav id='nav-elements-container' className="flex uppercase font text-m font-normal justify-start">
                ...
                <motion.div 
                    ...
                    className={`text-ivory font-bold text-xxl bg-offblack absolute top-10 left-0 flex flex-col justify-start w-full sm:hidden ${isActive ? 'block' : 'hidden'} rounded-b-[1.5em] h-[10em] mt-4`} 
                    initial={{ y: -50, opacity: 0 }} 
                    animate={{ y: isActive ? 0 : -0, opacity: isActive ? 1 : 0 }} 
                    transition={{ duration: 0.3 }}>
                    
                    <Link to="/" className='px-5 py-1 sm:hover:text-offblack sm:hover:opacity-40'>Home</Link>
                ...
                <div id='links-container' className={`font-semibold text-ivory right-0 top-full ml-auto ${isActive ? "hidden" : "hidden"} sm:flex justify-end gap-3`}>
                    <Link to="/" className='px-3 py-1 sm:hover:text-offblack sm:hover:opacity-40'>Home</Link>
